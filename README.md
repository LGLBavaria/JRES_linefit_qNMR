...
* `JRES_CoMList` (data frame): List of pre-identified Centers of Mass. Generated by `JRES_calculate_CoM()`.
* `all_base_areas_df` (data frame): Dataframe of all individual points belonging to any coherent component. Generated by `JRES_calculate_CoM()`.
* `target_f1_val` (numeric): The F1 ppm value of the target position for selection.
* `target_f2_val` (numeric): The F2 ppm value of the target position for selection.

The function calculates the Euclidean distance between each provided CoM and the specified target F1 and F2 ppm values. It then selects the CoM with the minimum calculated distance. Subsequently, it filters the `all_base_areas_df` to return only the points that belong to the selected CoM's cluster. It also includes logical error flags to indicate specific conditions: `Error_JRES0` is set if no CoMs were found in the input list, and `Error_JRES1` is set if more than one CoM was present in the input list, even though the closest one was selected.

#### Returns a list containing:

* `selected_com_point_df` (data frame): Includes the F1ppm, F2ppm coordinates, and total Intensity of the selected Center of Mass.
* `selected_base_area_df` (data frame): All points (F1ppm, F2ppm, Intensity, ComponentID) belonging to the selected coherent component.
* `Error_JRES0` (logical): Error flag, `TRUE` if `JRES_CoMList` has no entries.
* `Error_JRES1` (logical): Error flag, `TRUE` if `JRES_CoMList` has more than one entry.
