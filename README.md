...
[Analytical Chemistry – DOI 10.1021/acs.analchem.5c07171](https://pubs.acs.org/doi/10.1021/acs.analchem.5c07171)
...
**`ppm_f2_max`** (numeric): Maximum F2 ppm value for the region of interest.<br>
**`ppm_f1_min`** (numeric, optional): Minimum F1 ppm value for the region of interest, in ppm (default: -0.1).<br>
**`ppm_f1_max`** (numeric, optional): Maximum F1 ppm value for the region of interest, in ppm (default: 0.1).<br>
**`intensity_threshold`** (numeric, optional): Only intensities above this threshold will be considered for CCA and CoM calculation (default: 1000).<br>
The function first extracts a sub-spectrum based on the given F1 and F2 ppm range. It then filters the data points to include only those with intensities above the `intensity_threshold`. A Breadth-First Search (BFS) based Connected-Component Algorithm is applied to these filtered points to identify coherent signal regions. For each coherent component found, the Center of Mass (CoM) is calculated based on its F1ppm, F2ppm coordinates, and the sum of intensities within that component. The function also handles cases where no data points are found within the specified PPM range or no intensities exceed the threshold, returning empty results for the CoMs and base areas in such scenarios.<br><br>
**Returns a list containing:**<br>
`sub_spectrum` (data frame): A dataframe representation of the extracted spectral sub-region (matrix).<br>
`JRES_CoMList` (data frame): A dataframe where each row represents a distinct coherent component's Center of Mass, including F1ppm, F2ppm, total 'Intensity' (sum of intensities in the component), and a unique 'ComponentID'. Returns an empty dataframe if no components are found.<br>
`all_base_areas_df` (data frame): A dataframe of all individual points (F1ppm, F2ppm, Intensity, ComponentID) that belong to any coherent component identified. This can be used for plotting the base areas of all found components.<br><br>  

### JRES_select_target_CoM()
This function identifies the Center of Mass (CoM) that is closest to a specified target F1 and F2 ppm position from a given list of CoMs. It also retrieves the associated base area points for the selected CoM's coherent component.<br><br>
**Parameters:**<br>
**`JRES_CoMList`** (data frame): List of pre-identified Centers of Mass. Generated by `JRES_calculate_CoM()`.<br>
**`all_base_areas_df`** (data frame): Dataframe of all individual points belonging to any coherent component. Generated by `JRES_calculate_CoM()`.<br>
**`target_f1_val`** (numeric): The F1 ppm value of the target position for selection.<br>
**`target_f2_val`** (numeric): The F2 ppm value of the target position for selection.<br>

The function calculates the Euclidean distance between each provided CoM and the specified target F1 and F2 ppm values. It then selects the CoM with the minimum calculated distance. Subsequently, it filters the `all_base_areas_df` to return only the points that belong to the selected CoM's cluster. It also includes logical error flags to indicate specific conditions: `Error_JRES0` is set if no CoMs were found in the input list, and `Error_JRES1` is set if more than one CoM was present in the input list, even though the closest one was selected.<br><br>
**Returns a list containing:**<br>
`selected_com_point_df` (data frame): Includes the F1ppm, F2ppm coordinates, and total Intensity of the selected Center of Mass.<br>
`selected_base_area_df` (data frame): All points (F1ppm, F2ppm, Intensity, ComponentID) belonging to the selected coherent component.<br>
`Error_JRES0` (logical): Error flag, `TRUE` if `JRES_CoMList` has no entries.<br>
